...
export const LocationListForFilter = ({ selectedLocationId, setSelectedLocationId }) => {
    ...
    useEffect(() => {
      const getData = async () =>{
        ...
        // console.log("received_data from Location List page:", received_data)
  ...
  return (
    <>
        {/* length of location array: {lList.length} */}
        {
            lList.length ? lList.map((location)=>(
                ...
                // id, location, url, selectedLocationId, setSelectedLocationId
            )) : "locations is empty"
        }
    </>
  )
